...
namespace SpotifyLib.Services
{
	...
	public class ServiceLocator
	{
		...
		/// </typeparam>
		/// <returns>
		/// The <see cref="ServiceLocator"/> cast to <typeparamref name="T"/>
		/// </returns>
		...
		/// </typeparam>
		/// <param name="factory">
		/// Factory that re-creates the object each time the service is resolved
		/// </param>
		public void Register<T>(
			Func<object> factory)
		{
		...
		/// The initial stored value, for when no factory is specified. If null, factories are relied on.
		/// </param>
		...
		/// </summary>
		/// <typeparam name="T">
		/// The type of the registered service to look for
		/// </typeparam>
		/// <returns>
		/// The registered service of type <typeparamref name="T"/>
		/// </returns>
		public T Resolve<T>()
		{
			var type = typeof(T);
		...
		/// </param>
		/// <returns>
		/// <see cref="bool"/> indicating if the service is registered
		/// </returns>
